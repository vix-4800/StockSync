<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    <div class="flex items-center rounded-lg" x-data="{
        ...
        @foreach ($buttons as $key => [$color, $label, $value])
            <button x-on:click="state = {{ $value ? 'true' : 'false' }}" x-bind:aria-pressed="state ? 'true' : 'false'"
                ...
                {{ $attributes->merge(
                        ...
                        'relative inline-block rounded-lg outline-none cursor-pointer text-base sm:text-sm sm:leading-6 text-center ps-3 pe-3 py-1.5 transition-all duration-200 ease-in-out disabled:pointer-events-none disabled:opacity-70',
                        'selectify-toggle-on' => $key === 'on',
                        'selectify-toggle-off' => $key === 'off',
                    ]) }}>
                {{ $label }}
            </button>
        @endforeach
